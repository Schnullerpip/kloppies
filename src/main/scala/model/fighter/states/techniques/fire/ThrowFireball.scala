...
case class ThrowFireball(caster:Fighter) extends Technique(caster) with Summoning{
  ...
  override val description = """Throws a single fireball in the direction, the character is looking at,
                              dealing 10 points of damage on contact."""
  override val manaUse: Int = Techniques.MANA_USE_TECHNIQUE_LEVEL_1
  override val image = ImageMatrix.matrices("fireball.png").head.head
